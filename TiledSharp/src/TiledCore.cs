...
namespace TiledSharp
{
	public abstract class TmxDocument
	{
		...
		protected XDocument ReadXml(string filepath)
		{
			...
			var manifest = Array.Empty<string>();
    ...
    [Serializable]
    public class PropertyDict : Dictionary<string, PropertyDict.TypeValueEntry>
    {
        public PropertyDict(XContainer xmlProp)
        {
            ...
            foreach(var p in xmlProp.Elements("property"))
            {
                ...
                if(p.Attribute("type") != null)
                {
                    ptype = p.Attribute("type").Value;
                }
    ...
    public class TmxImage
    {
        ...
        public TmxImage(XElement xImage, string tmxDir = "")
        {
            ...
            Trans = new(xImage.Attribute("trans"));
    ...
    public class TmxBase64Data
    {
        ...
        {
            var encoding = (string)(xData.Attribute("encoding") ?? xData.Parent?.Attribute("encoding"));
            var compression = (string)(xData.Attribute("compression") ?? xData.Parent?.Attribute("compression"));
			if(encoding != "base64")
			{
                throw new("TmxBase64Data: Only Base64-encoded data is supported.");
			}
			...
			{
                throw new("TmxBase64Data: Unknown compression.");
			}
        }
    }
}
