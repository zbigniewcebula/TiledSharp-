...
namespace TiledSharp
{
	public class TmxMap : TmxDocument
	{
		...
		private void Load(XDocument xDoc)
		{
			...
			var hexSideLength = xMap.Attribute("hexsidelength");
			if(hexSideLength != null)
			{
			    HexSideLength = (int)hexSideLength;
            }
            ...
            var nextObjectId = xMap.Attribute("nextobjectid");
            if(nextObjectId != null)
            {
                NextObjectID = (int)nextObjectId;
            }
            
            BackgroundColor = new(xMap.Attribute("backgroundcolor"));

            Properties = new(xMap.Element("properties"));

            Tilesets = new();
            foreach(var e in xMap.Elements("tileset"))
            {
                Tilesets.Add(new(e, TmxDirectory, CustomLoader));
            }
